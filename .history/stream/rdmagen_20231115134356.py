...
class RdmaGen(FlowGenerator):
    ...
    def run_rdma(self, flow : FlowInfo, output_file : str):
        # cmd = f"ib_write_bw -d {flow.server_ip} -p {flow.server_port} -s {size}"
        #
        #
        #
        #     HERERERERERRRRRRRRRRRREEEEEEEEEEEEEEEEEEEEEEEEER
        #
        #
        cmd = f"ib_write_bw --disable_pcie_relaxed -d {f.client_nic} 172.16.200.10 -p 50001  -s 100000  -n 5"
