...
class IperfGen(FlowGenerator):
    ...
    def run_iperf(self, server_ip, server_port, size, output_file):
        ...
        cmd = f"echo 'iperf -c {server_ip} -p {server_port} -n {size} -Z dctcp'" # use dctcp as the congestion control algorithm
        with open(output_file, 'w') as file:
            subprocess.run(cmd.split(), stdout=file, stderr=subprocess.STDOUT)

    def wait_for_all(self):
        for process in self.processes:
            if process.is_alive():
                process.join()
        print("All flows are finished.")
