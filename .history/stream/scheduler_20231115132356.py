...
class FlowScheduler:
    def __init__(self, network: dict, duration_s : float, fc : FlowCollection, p = 1):
        """
        ...
        if fc.type == 'tcp':
            self.flowGens = [IperfGen() for _ in range(p)]
        elif fc.type == 'rdma':
            self.flowGens = [RdmaGen() for _ in range(p)] 
           
    def setup_servers(self):
        for flowGen in self.flowGens:
            flowGen.setup_servers(self.fc.flows)
    
    def teardown_servers(self):
        for flowGen in self.flowGens:
            flowGen.teardown_servers()
