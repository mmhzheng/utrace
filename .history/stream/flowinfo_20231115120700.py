...
class FlowInfo:
    def __init__(self, id: int, client : str, server_ip : str, server_port : str, size: int):
        self.id = id
        self.client = client
...
class FlowCollection:
    def __init__(self, network: dict, client : str, server_ip : str, server_port_base : int, 
    ...
    def get_server_port_base(self) -> int:
        return self.server_port_base
    
    # @output: the number of added flows
    def generate_flows(self) -> None:
        for i in range(self.num):
            flow = FlowInfo(i, self.network[self.client], self.network[self.server_ip], self.server_port_base + i, self.s[i])
