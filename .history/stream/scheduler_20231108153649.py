    ...
    def start(self) -> int:
        ...
        with tqdm(total=len(self.flows)) as pbar:
            ...
            for i, f in enumerate(self.flows[thread_id]):
                elapsed_time = time.perf_counter() * S_TO_US - start_time
                expected_time = start_time + (self.duration_perf_count * (i+1) / len(self.flows[thread_id]))
                remaining_time = expected_time - elapsed_time
                while time.perf_counter() < rest_time:
                    pass  
